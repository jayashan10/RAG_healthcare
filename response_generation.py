...
def generate_response(chunks: List[dict], query: str, chat_history: List[str]) -> dict:
    ...
    context = ' '.join([chunk['text'] for chunk in chunks[:3]])  # Adjust based on your chunk sizes and model's context window
    ...
    sources = [
        ...
        for chunk in chunks[:3]  # Include sources for top 3 chunks
    ]
